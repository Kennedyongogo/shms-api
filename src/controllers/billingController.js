...
const generateBill = async (req, res) => {
  try {
    ...
    return res.status(500).json({ success: false, message: "Error generating bill", error: error.message });
  }
};
...
const addBillItems = async (req, res) => {
  try {
    ...
    return res.status(500).json({ success: false, message: "Error adding bill items", error: error.message });
  }
};
...
const getByReference = async (req, res) => {
  try {
    ...
    const status = await getBillingStatusByReference({ item_type, reference_id });
    if (!status.ok) {
      return res.status(400).json({ success: false, message: status.message || "Invalid request" });
    }
    return res.status(200).json({ success: true, data: status });
  } catch (error) {
    return res.status(500).json({ success: false, message: "Error fetching billing status", error: error.message });
  }
};
